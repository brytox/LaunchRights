...
#ifndef LAUNCHRIGHTSSHARED_SWIFT_H
#define LAUNCHRIGHTSSHARED_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="LaunchRightsShared",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_PROTOCOL("_TtP18LaunchRightsShared14HelperProtocol_")
@protocol HelperProtocol
/// Ask the daemon to (re)launch an allowlisted app with elevated (root) rights.
/// \param bundlePath absolute path to the <code>.app</code> the agent observed launching.
...
///
/// \param reply <code>(success, message)</code> — message is human-readable for logs/UI.
///
- (void)launchElevatedWithBundlePath:(NSString * _Nonnull)bundlePath withReply:(void (^ _Nonnull)(BOOL, NSString * _Nonnull))reply;
/// Liveness / version probe. Returns a short status string.
- (void)pingWithReply:(void (^ _Nonnull)(NSString * _Nonnull))reply;
...
/// directly.
- (void)recentEventsWithLimit:(NSInteger)limit withReply:(void (^ _Nonnull)(NSArray<NSString *> * _Nonnull))reply;
@end

@class NSXPCListener;
@class NSXPCConnection;
/// The XPC surface both daemons expose (<code>com.jigsaw24.launchrights.helper</code>):
...
/// the XPC prototype (<code>launchrightsd</code>) or the Endpoint Security daemon
/// (<code>launchrightsd-es</code>) — with no client-side changes. Only one flavour registers
/// the Mach service at a time (don’t run both).
SWIFT_CLASS("_TtC18LaunchRightsShared13HelperService")
@interface HelperService : NSObject <NSXPCListenerDelegate, HelperProtocol>
