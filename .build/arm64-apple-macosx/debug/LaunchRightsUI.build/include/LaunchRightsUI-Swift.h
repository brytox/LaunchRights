...
#ifndef LAUNCHRIGHTSUI_SWIFT_H
#define LAUNCHRIGHTSUI_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="LaunchRightsUI",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class NSMenu;
/// Shared status-bar controller used by both the standalone <code>launchrights-menu</code>
/// executable and the System Extension host app.
/// Lives in its own target (not <code>LaunchRightsShared</code>) so the root daemons never link
/// AppKit. Callers can inject extra menu items via <code>extraItems</code> — the host app
/// uses this to add System Extension activate/deactivate controls.
SWIFT_CLASS("_TtC14LaunchRightsUI14MenuController")
@interface MenuController : NSObject <NSApplicationDelegate, NSMenuDelegate>
